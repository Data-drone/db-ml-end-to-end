...
# MAGIC
# MAGIC When you score a model trained with features from time series feature tables, the appropriate features are retrieved using point-in-time lookups with metadata packaged with the model during training. The DataFrame you provide to `FeatureStoreClient.score_batch` must contain a timestamp column with the same name and DataType as the `timestamp_lookup_key` of the FeatureLookup provided to `FeatureStoreClient.create_training_set`.
